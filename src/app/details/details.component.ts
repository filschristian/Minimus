...
export class DetailsComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.activeRouter.paramMap.subscribe((route: any) => {
      ...
      this.sub1 = this.weather.getWeather(this.city).pipe(first()).subscribe((payload: any) => {
        this.state = payload.weather[0].main;
        this.temp = Math.ceil(payload.main.temp);
        this.wind = Math.ceil(payload.wind.speed);
        this.hum = Math.ceil(payload.main.humidity);
      });
      this.sub2 = this.weather.getForecast(this.city).subscribe((data: any) => {
